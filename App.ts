...
// User type aligning with the users table
interface User {
  user_id: number;
  username: string;
  email: string;
...
function App() {
  ...
  const [user, setUser] = useState<User | null>(null); // Store user metadata
  ...
  const handleSignIn = async () => {
    try {
      const response = await axios.get<User>("http://localhost:3006/api/auth/user", {
        headers: {
